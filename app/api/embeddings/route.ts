...
import { embeddings, LiteLLMConfigError, LiteLLMRequestError } from "@/lib/litellm";
import { checkRateLimit, getClientIdentifier } from "@/lib/rate-limit";
import type { EmbeddingsRequestBody } from "@/types/chat";
...
export async function POST(req: NextRequest) {
  ...
  try {
    const upstream = await embeddings({ model: body.model, input: body.input }, req.signal);
    ...
    if (err instanceof LiteLLMRequestError) {
      return Response.json({ error: "Embeddings provider error." }, { status: err.status >= 500 ? 502 : err.status });
    }
